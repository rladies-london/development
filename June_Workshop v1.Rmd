...
i.e. add-on functionality built for R but not pre-installed (base R)
...
head(df, n=2) #shows first parts of object, here requesting the first 2 rows
tail(df, n=2) #shows last parts of object, here requesting the last 2 rows
...
df[c(df$V1, df$V2...)] #function arguments: df=data frame, in square brackets specify columns to select or deselect

#AIM: select the first 2 columns in the painters data frame
painters[c(1,2)] #subset the df, taking just the 1st & 2nd columns (and all the rows)

#AIM: drop the 4th column in the painters data frame, Expression
painters[-4] #returns the subsetted df with the 4th column, Expression removed

#select & deselect based on variable values
df[which(),] #df=data frame, specify the variable value within the which() to subset the df on, then ',' to denote all rows returned

#AIM: select all rows where the painters' School is the 'A' category
painters[which(painters$School == "A"),] #returns the subsetted df where equality holds true, i.e. value in School variable column is 'A'

#AIM: deselect all rows where the painters' School is the 'A' category, i.e. return df subset without 'A' values
painters[which(painters$School != "A"),] #returns the subsetted df where equality holds true, i.e. value in School variable column is not 'A'
```
...
takehome.pay(tax.rate = 0.2, income = 25000) #call our function to calculate 'takehome.pay' on a 'tax.rate' of 20% and an 'income' of 25k
```
