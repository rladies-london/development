...
```
> Use this document to follow along during the session, published version on RPubs is here: xxxxx    

> This is *NOT* intended to be fully comprehensive list of every useful R function, rather a demonstration of selected examples. For a wider collection to work through, this Reference Card is recommended: https://cran.r-project.org/doc/contrib/Baggott-refcard-v2.pdf    

> Additional CRAN reference cards and R material found here: https://cran.r-project.org/other-docs.html    

### Schedule  

####1st Half (to be updated)
-- 6.30-6.35pm: A. Essentials    
-- 6.35-7.15pm: B. Basics    
-- 6:40-6:50pm: C. Core    
-- 6:50-6:55pm: D. Common    
-- 6:55-7:15pm: E.     

####Break  
-- 7:15-7.30pm        

####2nd Half (to be updated)
-- 6.30-6.35pm: A. Essentials    
-- 6.35-6.40pm: B. Basics    
-- 7:30-6:50pm: C. Core    
-- 6:50-8:15pm: D. Common    
-- 6:55-7:15pm: E. Further Functions
...
i.e. functionality built for R but not pre-installed, referred to as 'base R'
...
```{r 4, eval=FALSE}
#Add comments within your code using a hashtag
```

---    

#### 5. Load datasets from base R & called libraries - `data(foo)`
...
##### (d) Loading Non-Numerical Data - character strings
...
#### 9. Vector & Matrix Construction - `c()`, `cbind()`, `rbind()`
Vectors & Matrices are very common R data structures.
```{r 9, eval = FALSE}
foo <- c(1, 2, 3, 4) #creates a vector by concatenating data 
oof <- c("A", "B", "C", "D") 
...
#### 11. Vector & Matrix Sort & Selection - `sort()`, `'vector'[]`, `'matrix'[]`

```{r 11, eval = FALSE}
jumble <- c(4, 1, 2, 3) 
sort(jumble) #this numeric vector is sorted ascending (default)
sort(jumble, decreasing = TRUE) #specify the decreasing arg to reverse default

mumble <- c( "D", "B", "C", "A")
sort(mumble) #this character vector is sorted ascending (default)
sort(mumble, decreasing = TRUE) #specify the decreasing arg to reverse default

jumble[1] #selects first value in our oof vector
tail[jumble, 1] #selects last value
...
```{r 12, eval = FALSE}
df <- data.frame("V1"=1:4, "V2"=c("A","B","C","D"), "V3"=5:8) #creates a data frame with 3 columns with 4 rows each

class(df) #check data frame object class
names(df) # returns column names
str(df) #see structure of data frame
summary(df) #returns basic summary stats 
View(df) #invokes spreadsheet-style viewer
head(df) #shows first parts of object
tail(df) #shows last parts of object
...
#### 13. Data Frame Select & Manipulate - `table()`
...
#### 14. Descriptive/Summary Stats Functions - `mean()`, `median()`, `sd()`, `var()`, `sum()`, `min()`, `max()`, `range()`
...
#### 16. Apply Functions - `lapply()` using `list()`
...
#### 18. Programming Tools - `if-statement`
`if-statement` used when certain computations are to execute only when a specific condition is met. Additionally other computations may execute when the condition is not met.
```{r 18, eval = FALSE}
if (condition.to.test < 10)
    {
    result.after.test = 'smaller'
    }else{
    result.after.test = 'bigger'
    }
#here we want to test if the object, 'condition.to.test' is smaller than 10. If it is smaller, another object, 'result.after.test' is assigned the value 'smaller'. Otherwise, the 'result.after.test' object is assigned the value 'bigger'

condition.to.test <- 5 #we specific what the condition actually is

result.after.test #return the result of our if-statement given the specific condition
```

---    

#### 19. Programming Tools - `for loop`
A `for loop` is a automation method for repeating (looping) a specific set of instructions for each element in a vector.
```{r 19, eval = FALSE}
for (i in 2010:2015)
{
  print(paste("In the year", i))
}
#here we want to print the same phrase but for each year between 2010 to 2015. 'i' stands for index - this loop executes the code chunk 'print(past("In the year", i)) for each index value

```

---    

#### 20. Programming Tools - `function()...`
User-programmed functions allow you to specify customised arguments and returned values.
```{r 20, eval = FALSE}
takehome.pay <- function(tax.rate, income)
{
  tax = tax.rate * income
  return(income - tax)
}
#here we create a simplified take-home pay calculator (single-band), called 'takehome.pay'. It uses two arguments, a 'tax.rate', and an 'income' level. The function in the curly braces {} instructs what the 'takehome.pay' function should do when it is called, namely, calculate the tax owed in an object 'tax', and then return the result of the 'income' object minus the 'tax' object

takehome.pay(tax.rate = 0.2, income = 25000)
```
...
#### 21. Strings - `grep()`, `tolower()`, `nchar()`

#### 22. Further Data Selection - `quantile()`, `cut()`, `which()`, `na.omit()`, `complete.cases()`, `sample()`

#### 23. Further Data Creation - `seq()`, `rep()`
